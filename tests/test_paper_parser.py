...
def test_paper_parser_llm_success():
    """LLM 解析成功"""
    mock_llm = MagicMock()
    mock_llm.chat_auto.return_value = MagicMock(content='{"research_area": ["AI", "NLP"], "method_type": "method", "paper_type": "application", "keywords": ["transformer", "attention"], "novelty": "new architecture", "application_domain": ["text mining"], "techniques": ["transformer"], "datasets": ["WikiSQL"], "evaluation_metrics": ["accuracy"], "novelty_type": "new_method"}')
...
def test_paper_parser_llm_parsing_error():
    ...
    mock_llm = MagicMock()
    mock_llm.chat_auto.return_value = MagicMock(content="无法解析的响应")
