...
2026-06-25: 加 LightGBM LambdaMART backend (lightgbm),listwise loss,
按 paper_id 自动 grouping,替换 logistic regression 作为 production LTR。
保留 sklearn/numpy backend 兼容历史模型(legacy 不删)。
...
# 2026-06-25: LightGBM LambdaMART backend。可选依赖,缺失时 backend=lightgbm
# 的 fit/predict 必须 fail-fast,不静默 fallback 到 LR (否则实验被无声篡改)。
try:  # pragma: no cover
...
class LearningToRanker:
    ...
    def __init__(
        ...
        # 2026-06-25: backend 可选 "auto" | "sklearn" | "numpy" | "lightgbm"。
        # auto: sklearn if available else numpy。lightgbm 必须显式指定。
        if backend == "auto":
            self._backend: str = "sklearn" if _HAS_SKLEARN else "numpy"
        elif backend in ("sklearn", "numpy", "lightgbm"):
            self._backend = backend
        else:
            raise ValueError(
                f"Unknown backend {backend!r}; must be auto/sklearn/numpy/lightgbm"
            )
    ...
    @staticmethod
    def _group_rows_by_paper(rows: List[dict]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """按 paper_id 把 rows 拆成 (X, y, group)。

        2026-06-25: LightGBM LambdaRank 需要 listwise grouping。group 是
        每个 paper 的样本数,顺序必须与 paper_id 在 rows 中**首次出现**的顺序一致
        (LightGBM 严格要求 contiguous groups matching group sizes 数组)。

        - rows 中每个 dict 至少有 ``paper_id`` 字段;缺失时归入 "_missing_" 组。
        - 返回 ``X: (n, d) float64``, ``y: (n,) int32``, ``group: (g,) int32``。
        - ``sum(group) == len(rows)``。
        """
        X = np.asarray([r["features"] for r in rows], dtype=np.float64)
        ...
        seen: Dict[str, int] = {}
        for r in rows:
            pid = r.get("paper_id") or "_missing_"
            if pid not in seen:
                seen[pid] = len(group)
                group.append(0)
            group[seen[pid]] += 1
        return X, y, np.asarray(group, dtype=np.int32)
    ...
    def fit(self, rows: List[dict]) -> "LearningToRanker":
        if self._backend == "lightgbm":
            return self._fit_lightgbm(rows)
    ...
    # ---- LightGBM LambdaMART backend (2026-06-25) ----

    def _fit_lightgbm(self, rows: List[dict]) -> "LearningToRanker":
        """LightGBM Lambdarank 训练路径。

        - 必须传 ``paper_id`` per-row(用于 listwise grouping)。
        - 不做特征标准化(LightGBM 内部用 histogram 处理 scale-invariant)。
        - num_threads=1 强制单线程以保证 determinism + 比特相等预测。
        """
        if not _HAS_LIGHTGBM:
            raise RuntimeError(
                "lightgbm backend requested but lightgbm is not installed. "
                "Install with: uv sync  (lightgbm>=4.0.0 is in pyproject.toml)."
            )
        # 即使 standardization=True 也对 lightgbm 无效 — sklearn scaler 不传
        self._scaler = None
        X, y, group = self._group_rows_by_paper(rows)
        self._feature_dim = X.shape[1]
        # 2026-06-25: store feature_names on ranker so save() can write it
        # to JSON. Without this, eval_ltr_on_holdout.py raises
        # RuntimeError("model has no feature_names").
        self._feature_names: Optional[List[str]] = (
            list(rows[0].get("feature_names") or []) if rows else None
        )

        train_set = _lgb.Dataset(X, label=y, group=group, free_raw_data=False)
        params = {
            ...
            "learning_rate": 0.05,
            "num_leaves": 31,
            # 2026-06-25: min_data_in_leaf 默认 1 而非 20。
            # 训练数据是 paper-grouped listwise,每个 group 通常只有 1 pos + 10 neg
            # (≈11 rows);min_data_in_leaf=20 > group size 时模型无法分裂,
            # booster 永远是常量 → predict 全 0。min_data_in_leaf=1 在大数据
            # 上也安全(bagging + L2 regularization 控制过拟合)。
            "min_data_in_leaf": 1,
            ...
            "verbose": -1,
            "seed": self.seed,
            "num_threads": 1,  # determinism + bit-equal
        }
        booster = _lgb.train(
            params,
            train_set,
        ...
        self.convergence_info = {
            "converged": True,
            "n_iter": int(booster.current_iteration()),
            "max_iter": self.max_iter,
            "warning_message": None,
            "objective": "lambdarank",
        }
    ...
    def _predict_lightgbm(self, X: np.ndarray) -> List[float]:
        """LightGBM inference: min-max normalize raw scores to [0,1]。

        注意:scores **不跨调用可比**(per-call min-max);只用于单个
        ``predict_scores`` 调用内的 candidate 排序。LTRAdapter 内部
        stable-sort,只关心 ordering,不关心绝对值。
        """
        # 2026-06-25: diagnostic path - detect NaN/Inf rows + per-row predict
        # with try/except, used to localize holdout240 segfault root cause.
        nan_inf_mask = ~np.isfinite(X).all(axis=1)
        if nan_inf_mask.any():
            print(
                f"[predict_lightgbm][DIAG] {nan_inf_mask.sum()}/{len(X)} rows "
                f"have NaN/Inf features; substituting neutral 0.5",
                flush=True,
            )
        raw = np.full(len(X), 0.5, dtype=np.float64)
        for i in range(len(X)):
            if nan_inf_mask[i]:
                continue
            try:
                raw[i] = float(self._model.predict(X[i:i+1])[0])
            except Exception as e:  # noqa: BLE001
                print(
                    f"[predict_lightgbm][DIAG] row {i} predict raised "
                    f"{type(e).__name__}: {e}",
                    flush=True,
                )
        if len(raw) > 1:
            ...
            if hi > lo:
                ...
                # 所有候选分数相同 → 退化为 0,排序退化到 stable input order
                probs = np.zeros_like(raw)
        else:
            # 单候选时 LR 返回 0.5 (中性),LightGBM 同样给中性值
            probs = np.array([0.5])
        return [float(p) for p in probs]
    ...
    def save(self, path: str) -> None:
        ...
        if self._backend == "lightgbm":
            booster_str = self._model.model_to_string()
            payload = {
                "schema_version": 1,
                "backend": "lightgbm",
                ...
                "lightgbm_booster_str": booster_str,
    ...
    def load(cls, path: str) -> "LearningToRanker":
        ...
        2026-06-25: 加 lightgbm backend 反序列化路径,旧 sklearn/numpy
        backend 行为不变(load 后统一走 numpy 推理避免 sklearn 循环依赖)。
        """
        payload = json.loads(Path(path).read_text(encoding="utf-8"))
        backend = str(payload.get("backend", "sklearn"))
        # 2026-06-25: lightgbm backend 必须保持原 backend 走 lightgbm 推理,
        # 不能 fallback 到 numpy(否则 booster 没意义)。
        if backend == "lightgbm":
            if not _HAS_LIGHTGBM:
                raise RuntimeError(
                    "lightgbm model loaded but lightgbm is not installed. "
                    "Install with: uv sync  (lightgbm>=4.0.0 is in pyproject.toml)."
                )
            ranker = cls(
            ...
            ranker._feature_names = payload.get("feature_names")
            ranker._model = _lgb.Booster(model_str=payload["lightgbm_booster_str"])
            ranker.convergence_info = payload.get("convergence_info")
            return ranker
        # legacy sklearn/numpy path
        ranker = cls(
            seed=int(payload.get("seed", 42)),
            max_iter=int(payload.get("max_iter", 1000)),
